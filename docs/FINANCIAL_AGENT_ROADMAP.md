...
样本池第一版设计见 `docs/superpowers/specs/2026-06-24-financial-backtest-ai-hardware-pool-design.md`，机器可读名单见 `agents/financial/backtests/sample_pool_v1.csv`。

1. 在 `agents/financial/backtests/` 增加：
   - 样本清单和格式说明；
   - 历史输入快照生成器；
   - 下一季度归母净利润同比方向标签生成器；
   - 回测执行器；
